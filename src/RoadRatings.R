...
Ratings2000 <- read.csv("data/RoadRatings2000.csv", stringsAsFactors = FALSE)


shapeData@data <- merge(shapeData@data, 
                        Ratings2000, 
                        by.x = "STREET_ID", 
                        by.y = "streetID")
...
plot(shapeData, 
     col = c("yellow", "green","red")[shapeData@data$status], 
     lwd = 2, 
     main = "2000 Road Ratings")
